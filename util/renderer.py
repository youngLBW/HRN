...
def set_rasterizer(type = 'pytorch3d'):
    if type == 'pytorch3d':
        ...
        import os
        from .deca_util import load_obj
...
class Pytorch3dRasterizer(nn.Module):
    ...
    def __init__(self, image_size=224):
        ...
        raster_settings = deca_util.dict2obj(raster_settings)
...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename='assets/3dmm_assets/template_mesh/template_bfm.obj', uv_size=256, rasterizer_type='pytorch3d'):
        ...
        # faces
        dense_triangles = deca_util.generate_triangles(uv_size, uv_size)
        ...
        face_uvcoords = deca_util.face_vertices(uvcoords, uvfaces)
        ...
        face_colors = deca_util.face_vertices(colors, faces)
    ...
    def render_depth(self, transformed_vertices):
        ...
        z = z/z.max()
        # Attributes
        attributes = deca_util.face_vertices(z, self.faces.expand(batch_size, -1, -1))
        # rasterize
        transformed_vertices[:,:,2] = transformed_vertices[:,:,2] + 10
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)
    ...
    def render_colors(self, transformed_vertices, colors):
        ...
        # Attributes
        attributes = deca_util.face_vertices(colors, self.faces.expand(batch_size, -1, -1))
        # rasterize
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)
        ####
